...
SQLALCHEMY_DATABASE_URI = os.environ.get(
    "REDASH_DATABASE_URL", os.environ.get("DATABASE_URL", "postgresql://postgres:admin@localhost:5432/redash")
)
...
CONTENT_SECURITY_POLICY = os.environ.get(
    ...
    "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-eval'; font-src 'self' data:; img-src 'self' http: https: data: blob:; object-src 'none'; frame-ancestors 'self' https://emanifest.mwan.gov.sa/ http://localhost:3000; frame-src redash.io;",
)
