...
def jwt_token_load_user_from_request(request):
    ...
    if org_settings["auth_jwt_auth_cookie_name"]:
        ...
        return None
    if org_settings["idp-login"] and request.args.get('token'):
        jwt_token = request.args.get('token')
    # logger.info("jwt_token " + jwt_token)
    # logger.info("expected_issuer " + org_settings["auth_jwt_auth_issuer"])
    # logger.info("expected_audience " + org_settings["auth_jwt_auth_audience"])
    # logger.info("algorithms " + "RS256")
    # logger.info("public_certs_url " + org_settings["auth_jwt_auth_public_certs_url"])
    if jwt_token:
...
def init_app(app):
    ...
    for blueprint in [create_google_oauth_blueprint(app), saml_auth.blueprint, remote_user_auth.blueprint, ldap_auth.blueprint, account.blueprint ]:
...
def create_and_login_user(org, name, email, picture=None):
    try:
        ...
        if user_object.name != name:
            logger.info("Updating user name (%r -> %r)", user_object.name, name)
            user_object.name = name
            models.db.session.commit()
    except NoResultFound:
        logger.info("Creating user object (%r)", name)
        user_object = models.User(
            ...
            group_ids=[org.default_group.id, org.admin_group.id],
        )
        models.db.session.add(user_object)
        models.db.session.commit()
    logger.info(org)
    login_user(user_object, remember=True, force=True)
    return user_object
